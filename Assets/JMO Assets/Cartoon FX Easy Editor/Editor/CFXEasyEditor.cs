...
public class CFXEasyEditor : EditorWindow
{
	...
	//----------------------------------------------------------------
		...
		//----------------------------------------------------------------
			...
			GUILayout.BeginHorizontal();
			if(GUILayout.Button(new GUIContent("Scale Size", "Changes the size of the Particle System(s) and other values accordingly (_speed, gravity, etc.)"), GUILayout.Width(120)))
			{
				applyScale();
			}
		...
		//----------------------------------------------------------------
			...
			GUILayout.BeginHorizontal();
			if(GUILayout.Button(new GUIContent("Set Speed", "Changes the simulation _speed of the Particle System(s)\n1 = default _speed"), GUILayout.Width(120)))
			{
				applySpeed();
			}
...
//		GUILayout.Space(6);
	...
	//----------------------------------------------------------------
		...
		if(copyFoldout)
		{
			...
			GUILayout.BeginHorizontal();
			if(GUILayout.Button(" Select Landwalk ", EditorStyles.miniButtonLeft))
			{
	...
	private void copyGradient(SerializedProperty ss, SerializedProperty sd)
	{
		...
		do
		{
			switch(gradient.propertyType)
			{
				...
				default: Debug.Log("CopyGradient: Unrecognized property _type:" + gradient.propertyType); break;
			}
			gradient.Next(true);
			copyGrad.Next(true);
		}
	...
	private void SubModuleCopy(SerializedObject source, SerializedObject dest)
	{
		...
		for (int i = 0; i < arraySize; i++)
		{
			if (source.FindProperty("SubModule.subEmitters.Array.data[" + i + "].emitter").objectReferenceValue != null)
			{
				...
				dest.FindProperty("SubModule.subEmitters.Array.data[" + i + "]._type").intValue = source.FindProperty("SubModule.subEmitters.Array.data[" + i + "]._type").intValue;
	...
	private void ScaleParticleValues(ParticleSystem ps, GameObject parent)
	{
		...
		{
			//(ShapeModule._type 6 == Mesh)
			if(psSerial.FindProperty("ShapeModule._type").intValue == 6)
			{
