...
public class STController : MonoBehaviour
{
    ...
    private void UpdateToolTipPosition(RectTransform tooltipWindow)
    {
        Vector2 anchoredPosition = Mouse.current.position.ReadValue();
        if (anchoredPosition.x + tooltipWindow.rect.width > Screen.width)
        {
            anchoredPosition.x = Screen.width - tooltipWindow.rect.width;
        }
        if (anchoredPosition.y + tooltipWindow.rect.height > Screen.height)
        {
            anchoredPosition.y = Screen.height - tooltipWindow.rect.height;
        }
        tooltipWindow.anchoredPosition = anchoredPosition;
    }
}
